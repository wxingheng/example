    ...
    <div class="p-a">
      <div>
        <div class="container">
          <flexbox>
            <flexbox-item>
              <div class="reg_res bg-info center-block">审证提交</div>
            ...
            <flexbox-item>
              <div
                v-if="stepFirst.result === 'Default'"
                class="reg_res reg_res_on center-block"
              >正在审核</div>
              <div
                v-if="stepFirst.result === 'Direct' | stepFirst.result === 'Pass'"
                class="reg_res bg-success center-block"
              >审核完成</div>
              <div
                v-if="stepFirst.result === 'UnPass'"
                class="reg_res bg-success center-block"
                style="line-height: 1.8rem;padding-top: 1rem;background-color: red"
              >审核
                <br>未通过
              </div>
              <span>{{stepFirst.auditTime && stepFirst.auditTime.substr(0, 10) || '--'}}</span>
              <br>
      ...
      <section class="container res_step_frame result_frame" style="margin-top:2rem;">
        <div v-if="stepFirst.result === 'Default'" class="row">
          <h5 class="text-center">您好，您的申请请求已提交！
            <br>我们将于 2个工作日之内予以答复
          </h5>
        ...
        <div v-if="stepFirst.result === 'UnPass'">
          <div v-if="stepFirst.caseType == '3'" class="row">
            ...
            <p style="text-indent:2em">您好，审核不通过。
              <br>
              {{stepFirst.unPassType}}, 请重新上传附件
              <br>
              请提供一下证明资料：{{stepFirst.unPassObj}}
              <br>
              {{stepFirst.auditRemark}}
            </p>
          </div>
