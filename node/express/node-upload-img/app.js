...
// docker 挂载使用绝对路径
// sudo docker run --name fed-fed-doc -v /nginx/html/fed/fed-doc:/usr/src/app/public -d -p 8080:3000 node-upload:v1.0
...
router.post('/upload', multipart(), function (req, res, next) {
  ...
  if (!/.*\.(jpg|gif|bmp|png|txt)$/ig.test(filename)) {
    return res.json({ code: 401, msg: '上传文件格式不被支持' });
  }
  var fi = filename.substr(filename.lastIndexOf('.')), targetPath;
  ...
  res.json({ code: 200, msg: { url: 'http://' + req.headers.host + '/images/' + newName } });
});
